...
def main() -> int:
    ...
    parser.add_argument("-f", help="Zero-based type field (default 2)",
                        type=int, default=2)
    parser.add_argument("type", nargs='+', type=string, required=True)
    # parser.add_argument("--")
    # parser.add_argument("<")
    parser.add_argument("UnicodeData.txt", required=True)
    ...
    force_compose = {}
    for lo, hi in force_compose:
        force_compose = [1 for ch in range(lo, hi)]

    # Only works for Python 3.7+
    date = run("date", capture_output=True, text=True).rstrip('\n')
    print(f"/* Generated by \"{sys.argv[0]} {sys.argv[1:]}\" on {date}.")
    print(" Python FTW! */")
...
def output(out: dict, code: int, type: str):
    ...
    elif (not out{in_run} or type ne out{run_type} or code != prev_code+1):
        end_run(out, prev_code)
